...
import { Menu, X } from 'lucide-react'; // Asegúrate de tener instalado: npm install lucide-react

// Aplicamos DRY: Centralizamos la data de navegación
// Al estar fuera del componente, evitamos que se re-cree en cada renderizado
const NAV_LINKS = [
  { id: 'home', label: 'Home' },
...
const NaviBar: React.FC = () => {
  ...
  // Función de flecha tipada para manejar el scroll suave
  const scrollToSection = (id: string): void => {
    const element = document.getElementById(id);
    if (element) {
      element.scrollIntoView({ behavior: 'smooth' });
      setIsMenuOpen(false); // Cierra el menú móvil automáticamente
    }
  };

  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#121212]/95 backdrop-blur-sm border-b border-gray-800">
      {/* Usamos un <nav> semántico con role para lectores de pantalla */}
      <nav 
